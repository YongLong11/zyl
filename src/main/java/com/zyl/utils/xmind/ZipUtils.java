...
//    private static ZipHomeProperties zipHomeProperties;
    ...
    public static String extract(InputStream inputStream) throws IOException, ArchiveException {
        //1. 解压缩之前的源文件
        String sourceName = ""; //目标文件夹名字
        String destFileName = ""; //目标文件夹名字
        String osName = System.getProperty("os.name");
        String path = "";
        path = "/app/logs";
        if (osName.startsWith("Mac")) {
            String relativelyPath = System.getProperty("user.dir");
            path = relativelyPath + "/null/";
        }
        sourceName = path + File.separator + "SrcXMind" + System.currentTimeMillis();
        log.info("源文件目录：{}", sourceName);
        File sourceFile = new File(sourceName);
        FileUtils.copyInputStreamToFile(inputStream, sourceFile);

        //2. 将源文件压缩至临时目录
        destFileName = path + File.separator + "XMind" + System.currentTimeMillis();
